...
def save_dataset():
    ...
    df['performance_metrics_parsed'] = df['performance_metrics'].apply(parse_metrics)

    mask_to_drop = df['performance_metrics_parsed'].apply(all_nan_metrics)
...
def categorize_model_type_detailed(input_filepath, output_filepath):
    try:
...
if __name__ == "__main__":
    try:
        categorize_model_type_detailed("../../assets/huggingface_co2_clean.csv",
                                       "../../assets/huggingface_co2_clean_pm.csv")
    except FileNotFoundError:
        print("[ERROR]")
